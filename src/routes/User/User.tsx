...
export default function Client({ setAuth }: ISetAuth) {
  ...
  const { surname, name, mail, oldPassword, newPassword1, newPassword2 } = inputs;

  const onSshChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const onSubmitChanges = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (oldPassword && newPassword1 && newPassword2) {
      if(newPassword1 !== newPassword2) {
        toast.error("Das neue PAsswort wurde nicht korrekt wiederholt!");
      } else {
        // API call für Passwort-Änderung
      }
    } else if(oldPassword || newPassword1 || newPassword2) {
      toast.warning("Wenn sie ihr Passwort ändern wollen, müssen sie alle Felder ausfüllen!");
    } else if (surname !== userInfo.surname || name !== userInfo.name || mail !== userInfo.mail) {
  ...
  const onInputChanges = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    e.preventDefault();
      ...
      <Container>
        ...
        <form onSubmit={(e) => onSubmitChanges(e)}>
          <Grid container direction="row" alignItems="center" justify="flex-start" spacing={2}>
            ...
            <Grid item xs={12}>
              <Typography>
                Um ihr Passwort zu ändern, gaben sie bitte ihr Passwort ein, suche sie sich ein Neues aus und wiederholen sie es.
              </Typography>
            </Grid>
            <Grid item xs={12}>
              <TextField
          ...
          <Grid container direction="row" justify="flex-end" alignItems="center" spacing={2}>
            <Grid item>
              <Button variant="contained" color="default" className={classes.button} onClick={resetInputs}>
                Reset
            </Button>
            </Grid>
            <Grid item>
        ...
        <form onSubmit={(e) => onSshSubmit(e)}>
          ...
          <Grid container direction="row" alignItems="center" justify="flex-end" spacing={2}>
            <Grid item>
              <Button variant="contained" color="default" className={classes.button} onClick={resetSshKeys}>
                Reset
            </Button>
            </Grid>
            <Grid item>
